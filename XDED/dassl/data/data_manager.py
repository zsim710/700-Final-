...
from .transforms.audio_transforms import get_audio_transforms
...
def build_domain_balanced_data_loader(
    cfg,
    sampler_type='SequentialSampler',
    data_source=None,
    batch_size=64,
...
class DomainBalanced_DataManager(DataManager):
...
class AudioDatasetWrapper(TorchDataset):
    ...
    def __init__(self, cfg, data_source, transform=None, is_train=False):
        ...
        try:
            from .transforms.audio_transforms import get_audio_transforms
            self.audio_transform = get_audio_transforms(
                mode='train' if is_train else 'test',
                sample_rate=16000,
                max_length=16000  # 1 second at 16kHz
            )
        except ImportError as e:
            print(f"Warning: Could not import audio transforms: {e}")
    ...
    def __getitem__(self, idx):
        ...
        output = {
            'label': item.label,
            'domain': item.domain,
            'impath': item.impath,  # Keep for debugging
            'classname': getattr(item, 'classname', ''),
            'utterance_id': getattr(item, 'utterance_id', f'utt_{idx}')
        }

        # Load audio instead of image
        try:
            if self.audio_transform is not None:
                audio = self.audio_transform(item.impath)  # Load and preprocess audio
            else:
                # Fallback: basic audio loading
                import torchaudio
                audio, sr = torchaudio.load(item.impath)
                if sr != 16000:
            ...
            output['img'] = audio  # Use 'img' key for compatibility with existing code
            
        except Exception as e:
            print(f"Error loading audio {item.impath}: {e}")
            # Return silent audio if loading fails
            output['img'] = torch.zeros(16000)
