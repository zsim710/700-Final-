...
def dataio_prepare(hparams):
    ...
    sb.dataio.dataset.set_output_keys(
        datasets, ["id", "sig", "wrd", "tokens_bos", "tokens_eos", "tokens", "wav"],
    )
...
if __name__ == "__main__":
    ...
    if not os.path.exists(raw_samples_path):
        ...
        # hook for other init
        os.makedirs(os.path.join(hparams["output_folder"], "wandb"))
        ...
        print("created ckpt link to", ckpt)
    ...
    # Testing
    wer_folder = os.path.join(hparams["output_wer_folder"], "test")
    if not os.path.exists(wer_folder):
        os.makedirs(wer_folder)

    for k in test_datasets.keys():  # keys are test_clean, test_other etc
        asr_brain.hparams.test_wer_file = os.path.join(
            wer_folder, f"wer_{k}.txt"
        )
        asr_brain.evaluate(
            test_datasets[k],
            max_key="ACC",
